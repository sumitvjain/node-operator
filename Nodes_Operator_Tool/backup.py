...
class NodeOperator(QWidget):
    ...
    def btn_import_clicked(self):
        ...
        if file_items:
            # sel_file_name = nuke.Enumeration_Knob('Exported File Selector', 'Select File Name' , file_items )
            # print('sel_file_name -- ', sel_file_name)
...
import sys
from PySide2.QtWidgets import QApplication, QMainWindow, QVBoxLayout, QWidget, QPushButton, QTextEdit, QLineEdit, QLabel, QHBoxLayout
from PySide2.QtCore import QThread, Signal
from shotgun_api3 import Shotgun

# ShotGrid Configuration
SHOTGUN_URL = "https://yourshotgridsite.shotgunstudio.com"
SHOTGUN_SCRIPT_NAME = "your_script_name"
SHOTGUN_API_KEY = "your_api_key"

# Worker Thread Class for Fetching Published Paths
class FetchPathsThread(QThread):
    progress = Signal(str)  # Signal to log messages in UI
    ...
    def run(self):
        try:
            self.progress.emit("Connecting to ShotGrid...")
            sg = Shotgun(SHOTGUN_URL, SHOTGUN_SCRIPT_NAME, SHOTGUN_API_KEY)
            self.progress.emit(f"Fetching published paths for sequence: {self.sequence_name}")

            # Query ShotGrid for Published Files in the given Sequence
            filters = [["entity.Sequence.code", "is", self.sequence_name]]
            fields = ["path", "name", "version_number"]
            results = sg.find("PublishedFile", filters, fields)

            if not results:
                self.progress.emit(f"No published paths found for sequence: {self.sequence_name}")
            else:
                for result in results:
                    path = result["path"]["local_path"] if result["path"] else "No path available"
                    name = result["name"]
                    version = result["version_number"]
                    self.progress.emit(f"Name: {name}, Version: {version}, Path: {path}")
        except Exception as e:
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        # Widgets
        self.label = QLabel("Enter Sequence Name:")
        self.sequenceInput = QLineEdit()
        self.logArea = QTextEdit()
        self.logArea.setReadOnly(True)
        self.fetchButton = QPushButton("Fetch Paths")
        self.fetchButton.clicked.connect(self.start_fetching)

        # Layout
        inputLayout = QHBoxLayout()
        inputLayout.addWidget(self.label)
        inputLayout.addWidget(self.sequenceInput)

        mainLayout = QVBoxLayout()
        mainLayout.addLayout(inputLayout)
        mainLayout.addWidget(self.fetchButton)
    ...
    def start_fetching(self):
        sequence_name = self.sequenceInput.text().strip()

        if not sequence_name:
            self.log("Please enter a sequence name.")
            return

        self.logArea.clear()
        self.log(f"Starting to fetch paths for sequence: {sequence_name}")

        # Start FetchPathsThread
        self.fetchButton.setEnabled(False)
        self.thread = FetchPathsThread(sequence_name)
        self.thread.progress.connect(self.log)
        self.thread.finished.connect(self.on_fetch_complete)
        self.thread.start()

    def on_fetch_complete(self):
        self.log("Fetching completed.")
